...
export default {
  ...
  setup (props, { emit }) {
    const state = reactive({
      ...
      contentValue: props.editorData, // 绑定文本
      timeout: null,
    })
    onMounted(() => {
      tinymce.init({})
    ...
    watch(() => state.contentValue, (n) => {
      debounce(() => {
        emit('update:editorData', state.contentValue)
      })
    })
    // 侦听默认值 外界第一次传进来一个 v-model 就赋值给 contentValue
    watch(() => props.editorData, (n) => {
    ...
    function imgUploadFn (blobInfo, success, failure) {
      ...
      // 图片上传
      axios.post('system-service/aSysFile/filesUpload', params, config).then(res => {
        if (res.data.code == 0) {
